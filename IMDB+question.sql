...
-- Rows in Movie Table = 7997

Select count(*) from genre;
-- Rows in Genre Table = 14662

Select count(*) from director_mapping;
-- Rows in director_mapping Table = 3867

Select count(*) from role_mapping;
-- Rows in role_mapping Table = 15615

Select count(*) from names;
-- Rows in names Table = 25735

Select count(*) from ratings;
-- Rows in names Table = 7997
...
-- In movies table following 4 columns have null values namely
-- country - 20 null values
-- worlwide_gross_income - 3724 null values
-- lanuages - 194 null values
...
-- month of january has most number of releases
...
select count(title) as no_of_movies from movie 
where ( country like '%INDIA%' or country like '%USA%' ) and year = 2019;

-- Number of movies produced in India or USA in ther year 2019 are 1059
...
-- 4285 drama movies were produced over all which is highest among all genres
...
with genre_count as 
(
select movie_id, count(distinct(genre)) as count_genre
from genre
group by movie_id
having count_genre = 1
)
select count(movie_id) as number_of_movies from genre_count;

-- There are 3289 movies that belong to  only 1 genre.
...
-- Average duration of movies is in range 92.72 - 112 .88 mins.
-- Action genre movies having highest duration and horror movies having lowest duration.
...
-- Dream Warrior Pictures and National Theatre Live production houses have produced most hit movies (average rating > 8).
...
-- Top 3 genres are drama, comedy and action for month of March 2017 in the USA that have more than 1,000 votes.
...
-- 361 movies were released between April 2018 to April 19 that have a median rating of 8.
...
order by movie_count desc limit 2;
 ...
 -- Mammootty and Mohanlal have the highest numebr of movies with 8 above rating
...
(
select
m.production_company, sum(r.total_votes) as vote_count, Rank() over (order by Sum(total_votes) desc) as prod_comp_rank
from movie m
inner join ratings r on m.id = r.movie_id
group by m.production_company
order by vote_count desc
)
select * from production_ranking 
where prod_comp_rank<=3;

-- Top three production houses based on votes received are Marvel Studios, Twentieth Century Fox and Warner Bros.
...
sum(avg_rating*total_votes)/sum(total_votes) as actor_avg_rating, Rank() over(order by sum(avg_rating*total_votes)/sum(total_votes) desc) as actor_rank  
from names n
...
-- Vijay Sethupathi,  Fahadh Faasil and Yogi Babu make the top 3 actors wth average rating 7.8 +
...
select n.name as actress_name, sum(total_votes), count(r.movie_id) as movie_count, sum(avg_rating*total_votes)/sum(total_votes) as actress_avg_rating, 
...
else 'Invalid' end as 'avg_rating_category'
from movie m
inner join ratings r on m.id = r.movie_id
inner join genre g on m.id = g.movie_id
where genre = 'thriller'; 
...
-- Star cinema and Twentieth Century Fox has highest count of movies with multilingual movies
...
count(r.movie_id) as movie_count, sum(avg_rating*total_votes)/sum(total_votes) as actress_avg_rating, 
dense_rank() over(order by sum(avg_rating*total_votes)/sum(total_votes) desc) as actoress_rank
from names n
inner join role_mapping rm on n.id = rm.name_id
...
-- Sangeetha, Fatima and Adriana are the top three actresses 
...
(
select d.name_id, NAME, d.movie_id, duration, avg_rating, total_votes, date_published,
...
select name_id as director_id, name as director_name, count(movie_id) as number_of_movies, avg(date_difference) as avg_inter_movie_days, 
avg(avg_rating) as avg_rating, sum(total_votes) as total_votes, min(avg_rating) as min_rating, max(avg_rating) as max_rating, sum(duration) as total_duration
from top_directors
